...
Respuesta:
```
db.film.aggregate([ { $lookup: { from: "language", localField: "language_id", foreignField: "_id", as: "idioma" } }, { $unwind: "$idioma" }, { $group: { _id: "$idioma.name", count: { $sum: 1 } } }, { $project: { _id: 1, count: 1 } }])
```
